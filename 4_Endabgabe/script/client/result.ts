namespace end_result {
    ...
    let url: string;
    let time: string = localStorage.getItem("MemoryTime");
    let currentID: string = localStorage.getItem("currentID");
    ...
    function sendClickEvent(): void {
        ...
        if (currentID != null) { isFirst = false; }
    ...
    async function sendData(_isFirst: boolean): Promise<void> {
        setURL();
    ...
    function checkUnicode(s: string): boolean {
        return /[^\u0000-\u00ff]/.test(s);
    }
